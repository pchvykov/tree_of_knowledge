...
ToK = function (svg, db) {
  console.log("ToK constructor called with svg:", svg, "db:", db);
  var tree = this;
  this.svg = svg;

  // Create db object if not provided
  if (!db) {
    db = {
      subscribe: function (visWindow, callback) {
        console.log("db.subscribe called with visWindow:", visWindow);
        // Call callback immediately for now
  ...
  var forceRun = false;
  console.log("Checking d3.scale:", d3.scale);
  ...
  this.redraw = function (postScript) {
    ...
    console.log("=== REDRAW DEBUG START ===");
    console.log("redraw() called with postScript:", postScript);
    console.log("Current visWindow:", visWindow);
    console.log("Current graph:", Session.get("currGraph"));
    console.log("Current zoom level:", Session.get("currZmLvl"));

    //store current node coordinates to restart from same position:
    if (force.nodes().length > 0) {
      console.log(
        "Storing coordinates for",
        force.nodes().length,
        "existing nodes",
      );
      Meteor.call("updateCoord", force.nodes());
    ...
    console.log("About to call db.subscribe with visWindow:", visWindow);
    db.subscribe(visWindow, function () {
      //parseInt($('#nnodesInput').val())
      console.log("=== db.subscribe callback START ===");
      console.log("db.subscribe callback called - redrawing");
      //this can access only published data - current graph:
      nodeData = Nodes.find().fetch();
      linkData = Links.find().fetch();
      console.log("Raw data received:");
      console.log("  nodeData count:", nodeData.length);
      console.log("  linkData count:", linkData.length);

      // Log detailed node information
      console.log("Detailed nodeData (first 3):");
      nodeData.slice(0, 3).forEach((node, i) => {
        console.log(`  Node ${i}:`, {
          _id: node._id,
          title: node.title,
          type: node.type,
      ...
      console.log("Processing nodeData...");
      var processedNodes = 0;
      var phantomNodes = 0;
      var unpositionedNodes = 0;
      ...
      console.log("  Phantom nodes:", phantomNodes);
      console.log("  Unpositioned nodes:", unpositionedNodes);
      ...
      console.log("=== D3 NODE BINDING DEBUG ===");
      console.log("Current node selection before binding:", node);
      console.log("About to bind", nodeData.length, "nodes to D3");
      ...
      console.log("  node.enter() size:", node.enter().size());
      console.log("  node.exit() size:", node.exit().size());
      console.log("  node.update() size:", node.size());
      ...
      console.log("Created", newNodes.size(), "new node groups");

      //choose node shape:
      console.log("Adding node shapes and event handlers...");
      newNodes
      ...
      console.log("Node shapes and event handlers added");
      ...
      console.log("Handling node exits and styling...");
      ...
      console.log("Styled", styledNodes.size(), "nodes");

      // Log first few styled nodes for debugging
      styledNodes.each(function (d, i) {
        if (i < 3) {
          console.log(`Styled node ${i}:`, {
            _id: d._id,
            title: d.title,
            type: d.type,
      ...
      console.log("=== FORCE GRAPH UPDATE ===");
      console.log("Setting force nodes:", nodeData.length);
      force.nodes(nodeData);
      // .links(linkData); //implemented by hand
      console.log("Starting force simulation...");
      force.start();
      force.alpha(0.06);
      console.log("Force alpha set to 0.06");

      // Final summary
      console.log("=== REDRAW SUMMARY ===");
      console.log("  Total nodes processed:", nodeData.length);
      console.log("  Total links processed:", linkData.length);
      console.log("  Force nodes count:", force.nodes().length);
      console.log("  SVG nodes in DOM:", d3.selectAll(".node").size());
      console.log("=== db.subscribe callback END ===");
  ...
  function tick(e) {
    // Debug logging every 10th tick to avoid spam
    if (Math.random() < 0.01) {
      console.log("=== TICK DEBUG (sample) ===");
      console.log("  alpha:", e.alpha);
      console.log("  nodeData count:", nodeData ? nodeData.length : 0);
      console.log("  linkData count:", linkData ? linkData.length : 0);
      if (nodeData && nodeData.length > 0) {
        console.log(
          "  Sample node positions:",
          nodeData.slice(0, 2).map((n) => ({
            _id: n._id,
            title: n.title,
            x: n.x,
            y: n.y,
    ...
    var positionedNodes = node.attr("transform", function (d) {
      var transform;
      ...
      if (
        ...
        transform = "translate(" + d.x + "," + d.y + ") rotate(" + rot + ")";
      }
    ...
    // Debug positioning results
    if (Math.random() < 0.01) {
      console.log("TICK: Positioned", positionedNodes.size(), "nodes in DOM");
    }
