...
GUI = function (tree) {
  ...
  this.showContent = function (d) {
    ...
    if (d.source) {
      gui.contentPopup = Blaze.renderWithData(
  ...
  this.showEditor = function (d, srcID) {
    console.log("=== showEditor DEBUG START ===");
    console.log("showEditor called with:");
    console.log("  d:", JSON.stringify(d, null, 2));
    console.log("  srcID:", srcID);
    console.log("  d.source exists:", !!d.source);

    if (gui.editPopup) {
      console.log("editPopup already exists, returning");
      return;
    }
    if (d.source) {
      //if a link is selected... edit
      console.log("Editing link");
      if (srcID) console.error("wrong inputs for showEditor");
      gui.editPopup = Blaze.renderWithData(
      ...
      //else it's a node
      console.log("Editing/creating node");
      ...
      console.log("Node edit popup created");
    }
    var offset = tree.svg.node().getBoundingClientRect();
    $("#editPopup").offset({
      top:
        offset.top + Math.round(tree.canvasSize[1] / 3) + 30 + window.scrollY,
      left: offset.left + 5 + window.scrollX,
    });
    $("#editPopup").css({
      "max-height": Math.round((tree.canvasSize[1] * 2) / 3) - 40 + "px",
    });
    //make text-area resize automatically (2nd argument to keep scrollbar in check:)
    autoSizeTextarea(document.getElementById("content"), $("#editPopup"));
    console.log("Popup positioned and styled, calling showContent");
    gui.showContent(d);
    console.log("=== showEditor DEBUG END ===");
  };
  ...
  this.nodeClick = function (d) {
    ...
    var hide = true;
    if (d != gui.selected) {
      //immediate response is not selected
      gui.showContent(d);
      hide = false;
    }
        ...
        if (
          tree.force
            ...
            .every(
              (lk) =>
                !(
                  (lk.source === mousedown_node ||
                    lk.target === mousedown_node) &&
  ...
  this.linkMouseup = function (d) {
    //Create new AND ("derivation") node:
    d3.event.stopPropagation();
    if (mousedown_node) {
      mouseup_link = d;
      if (
        mouseup_link.source !== mousedown_node &&
        mouseup_link.target !== mousedown_node
      ) {
        d3.event.stopPropagation(); //prevent anything else from happening
  ...
  this.nodeMouseout = function (d) {
    if (!d.dragging) {
      d3.select(this).classed("fixed", (d.fixed = d.permFixed));
      if (this.parentNode.tooltip)
  ...
  this.linkMouseover = function (d) {
    if (!(d.source.dragging || d.target.dragging)) {
      ...
      d.target.fixed = true;
      d3.selectAll(".node")
        .filter((dat) => dat._id == d.source._id)
      ...
      d3.selectAll(".node")
        .filter((dat) => dat._id == d.target._id)
  ...
  this.keydown = function () {
    if (d3.event.keyCode == 32 && !gui.editPopup) {
      //Spacebar pressed
      d3.event.preventDefault();
    ...
    if (d3.event.ctrlKey) {
      //editing controls accessed by holding Ctrl
      switch (d3.event.keyCode) {
        ...
        case 67: {
          //Ctrl+C - copy object data
          var obj = {};
          for (var attr in gui.selected) {
            //copy all but these prop-s:
            if (
              [
                "source",
                "target",
        ...
        case 86: {
          //Ctrl+V - paste obj data, not saved to DB!!
