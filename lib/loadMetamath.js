...
loadMetamath : function(thmList, mmCode){
	...
	//=================================================================
	readBlock(thmList.slice(0,2000),'',0); //Take first ... nodes
	//=================================================================
	...
	Nodes.find({$and:[{_id:{$in:unweighted}},{children:{ $nin: unweighted }}]},
		...
		{level:1}).forEach(function(nd, idx){ //take only level field
		...
		var parLk = Links.find({target : nd._id},{source:1});
		//============== Weigh parent links ==================
		...
		var parNChild = parLk.map(lk => //for each parent link, take
			Links.find({source: lk.source},{target:1,_id:0}) //all links with same parent,
