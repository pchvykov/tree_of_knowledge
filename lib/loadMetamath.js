...
export function parseMetamath(file) {
  var reader = new FileReader();
  reader.onload = function (e) {
    //when text file is read in
    ...
    for (var i = 0; i < mmString.length / 5; i++) {
      //traverse the file char by char
      ...
      if (mmToken && (!comment || char == ")")) {
        ...
        switch (char) {
          ...
          case ".": //end statement and store node
            if (stmtFlag == 2) {
              //parse proof string
              proof = proof.split(" ").filter(function (el) {
                return el.length != 0;
              });
              node.proof = proof.slice(
                proof.indexOf("(") + 1,
                proof.indexOf(")"),
              );

              var same = thmList[blkDepth].find(
                (nd) => nd.body == node.body && nd.tok == "p",
              );
              if (same) node.sameAs = same.label; //label if alternate proof
            }
            if (stmtFlag) {
              //add node object to array
...
// isArray and isObject are defined and exported at the top of this file
if (Meteor.isServer) {
  //else runs in parallel on both
  Meteor.methods({
    //load tree into DB directly on server
    loadMetamath: function (thmList, mmCode) {
      ...
      function readBlock(block, essHyp, stack) {
        //pass string of essential hypotheses from above
        var essLocal = essHyp;
        for (var ii in block) {
          //loop over entries of block
          console.log(
            "Loading " + ii + " of " + block.length + " at level " + stack,
          );
          var stmt = block[ii];
          if (isObject(stmt)) {
            //if it's really a statment (e or p)
            switch (stmt.tok) {
              case "e":
              ...
              case "f":
                ...
                if (stmt.tok == "p") {
                  ...
                  for (var ix in stmt.proof) {
                    ...
                    if (isNaN(lvl) || !lk.source) {
                      console.error("Bad link:", lk, stmt.proof[ix], nodeDic);
                      return;
                    }
                  }
                  ...
                  Nodes.update(
                    { title: { $in: stmt.proof } },
                    { $push: { children: ndID, chLevel: lvl } },
                    { multi: true },
                  );
                }
            }
          } else if (isArray(stmt)) {
            //else if array, then read recursively
...
// expose to global (for code that expects a global function)
if (typeof globalThis !== "undefined") {
  globalThis.parseMetamath = parseMetamath;
}
