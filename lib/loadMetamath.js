...
loadMetamath = function(file){
	...
	parseMetamath(file,function(thmList, mmCode){
							    ...
							    if(res){ 
							    	...
							    	if(stmt.tok=='p'){ //add links to dependencies
							    		for(var ix in stmt.proof){
							    			...
							    			lk.source = nodeDic[stmt.proof[ix]].id;
							    			lk.target = res[0];
							    			lk.oriented = true;
							    			lk.graph = "MetaMath";
							    			Meteor.call("updateLink", lk,
							    				function(err,res){
							    				  if(err) alert(err);
							    				});
							    		}
		...
		readBlock(thmList.slice(0,60),''); //Take first 60 nodes
		$('#availGraphs').append($('<option>', { 
              value: 'MetaMath',
              text : 'MetaMath' 
          }));
		
		Meteor.call("propagateWeights", 'MetaMath');

	});
}

//Compute node and link weights from leaves:
Meteor.methods({
	propagateWeights: function(graph){
		console.log("prop Weights!", graph)
	}
})