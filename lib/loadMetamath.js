...
loadMetamath : function(thmList, mmCode){
	console.log("Loading metamath");
	// var fileS = new File([JSON.stringify(thmList,null,1)],
	//   'parsed_MetaMath',
	//   {type: "application/json"});//"text/plain;charset=utf-8"});
	// saveAs(fileS);
	// console.log(thmList);
	var nodeDic = {}; //Dictionary giving node IDs and importance for every statement label
	...
	function readBlock(block, essHyp, stack){ //pass string of essential hypotheses from above
			...
			if(isObject(stmt)){ //if it's really a statment (e or p)
				switch(stmt.tok){
					...
					case 'a': 
						...
						nd.title=stmt.label;
						nd.text=essLocal+'=>\n'+stmt.body+'\n\\\\'+stmt.comment;
						nd.x=2500+100*Math.random(); nd.y=2500+100*Math.random(); //starting node location
						nd.graph='MetaMath';
		...
		if(stack==0){
			...
			Nodes.find({$and:[{children:{ $nin: unweighted }}, {_id:{$in:unweighted}}]})
			.forEach(function(nd){
				...
				var parLev = parLk.map(lk=>lk.level).reduce((sum, value) => sum + value,0);
				parLk.forEach(function(lk){
					Links.update(lk._id,{$set:{strength:(ndImp)/parLev*lk.level}});
				})
	...
	readBlock(thmList.slice(0,60),'',0); //Take first 60 nodes
